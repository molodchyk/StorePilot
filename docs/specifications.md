...
- Localized screenshot removal uses a `Remove Image` confirmation dialog. Do not batch-click multiple localized screenshot remove buttons. Click one localized screenshot remove button, confirm the modal, wait for the localized screenshot count to drop, then continue to the next remove button.
- While localized screenshot upload is running, StorePilot must not freeze or shield the CWS page. It should verify the selected dashboard locale before clearing and before each screenshot upload, and stop rather than writing to the wrong locale if the page changes underneath the operation.
